# Санарип Көз — Backend (слоистая архитектура)
...
## Зачем so модули Ollama, если они не используются на хакатоне

Папка `providers/ollama/` — не заглушка "для галочки". Это рабочая
референсная реализация: она ожидает на входе обычный Ollama-совместимый
REST API (`/api/chat`, аналог `/api/transcribe` для распознавания речи)
и должна заработать с минимальными правками после разворачивания на
сервере внутри периметра МВД, без доступа в интернет.
